...
if (code.includes("issueTableFields")) {
  console.log("Filter, Sort, and View Fields already appear to be installed.");
  process.exit(0);
}
...
code = code.replace(
  /  const filteredRows = useMemo\(\(\) => \{[\s\S]*?\n  \}, \[rows, searchText\]\);/,
...
code = code.replace(
  /        <div style=\{toolbarStyle\}>[\s\S]*?        <\/div>\n\n        \{selectedRowIds\.length > 0 \? \(/,
            ...
            <div style={fieldListStyle}>
              ...
              >
                <span>
                  ...
                  <small style={fieldMetaStyle}>Sort by newest issue first.</small>
                </span>
              </button>
...
code = code.replace(
  /                  <th style=\{thStyle\}>Issue<\/th>[\s\S]*?                  <th style=\{thStyle\}>Resolve \/ Close<\/th>/,
  `                  {visibleTableFields.map((field) => (
...
code = code.replace(
  /                  const linkedTask = getLinkedTask\(row\.linked_task\);\n\s*const linkedHuddle = getLinkedHuddle\(row\.link_to_daily_huddle\);\n/,
  ""
);

code = code.replace(
  /                      <td style=\{tdStyle\}>\s*\{renderEditableCell\(row, "issue_name"[\s\S]*?                      <td style=\{tdStyle\}>\s*\{renderEditableCell\(row, "resolve_close_date"[\s\S]*?<\/td>\n/,
  `                      {visibleTableFields.map((field) => (
